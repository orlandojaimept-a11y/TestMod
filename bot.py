...
# ── Catálogo ──────────────────────────────────────────────────────────────────
async def cb_catalog(update, ctx):
...
# ── Detalhe do produto ────────────────────────────────────────────────────────
async def cb_product(update, ctx):
    query = update.callback_query
    await query.answer()
...
async def cb_admin(update, ctx):
    ...
    if action == "close":
        ...
        if not orders:
            ...
            return
        lines = [
            f"• `{o['payment_id'][:12]}…` \\| {_esc(o['status'])} \\| "
            f"`{o['amount']} {o['currency']}` \\| uid:`{o['user_id']}`"
            for o in orders
        ]
        await query.edit_message_text(
...
async def cb_admin_del(update, ctx):
    query = update.callback_query
    await query.answer()
    pid = query.data.split(":")[1]
    remove_product(pid)
...
async def adm_add_start(update, ctx):
...
async def adm_recv_price(update, ctx):
    try:
        price = float(update.message.text.strip().replace(",", "."))
        if price <= 0:
            raise ValueError
    except ValueError:
        await update.message.reply_text("❌ Preço inválido\\. Digite um número positivo \\(ex: 9\\.99\\):", parse_mode="MarkdownV2")
        return AWAIT_PRICE
...
async def adm_recv_currency(update, ctx):
    query = update.callback_query
    await query.answer()
...
async def adm_recv_link(update, ctx):
    np = ctx.user_data["np"]
    np["link"] = update.message.text.strip()
...
async def adm_cancel(update, ctx):
    ctx.user_data.pop("np", None)
...
ptb_app.add_handler(CallbackQueryHandler(cb_admin,     pattern=r"^adm:(add|remove|orders|close|back)$"))
