...
async def cb_catalog(update: Update, ctx):
    ...
    await query.answer()
    log.info(f"cb_catalog | user={query.from_user.id} data={query.data}")

    try:
        page     = int(query.data.split(":")[1])
        products = await get_products()
        log.info(f"get_products retornou {len(products) if products else 0} itens")
    except Exception as e:
        log.error(f"Erro ao buscar produtos: {e}", exc_info=True)
        await query.edit_message_text("❌ Erro ao carregar produtos. Tente novamente.")
        return
    ...
    try:
        PAGE      = 3
        total_pgs = max(1, -(-len(products) // PAGE))
        ...
        log.error(f"Erro ao renderizar catálogo: {e}", exc_info=True)
        await query.edit_message_text("❌ Erro ao exibir catálogo. Tente novamente.")

# ── Detalhe do produto ────────────────────────────────────────────────────────
async def cb_product(update: Update, ctx):
    query = update.callback_query
    await query.answer()
    log.info(f"cb_product | user={query.from_user.id} pid={query.data}")
...
async def cb_buy(update: Update, ctx):
    ...
    log.info(f"cb_buy | user={query.from_user.id} pid={query.data}")
    ...
    try:
        ...
        log.error(f"Erro NowPayments: {e}", exc_info=True)
        await query.edit_message_text(
...
async def cb_admin(update: Update, ctx):
    ...
    if action == "close":
        ...
        orders = await list_orders(limit=ORDERS_DISPLAY_LIMIT)
        if not orders:
...
async def adm_recv_link(update: Update, ctx):
    ...
    np = ctx.user_data["np"]
    np["link"] = link
    await add_product(np["name"], np["description"], np["price"], np["currency"], np["link"])
    await update.message.reply_text(
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8000))  # Define 8000 como padrão se PORT não estiver setado
