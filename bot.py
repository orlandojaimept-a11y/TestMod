...
async def cmd_admin(update: Update, ctx):
    ...
    if not await _is_admin(update):
        ...
        return
    await _reply(update, "🔧 Painel Admin", reply_markup=make_admin_kb(lang))
...
async def cmd_addproduto(update: Update, ctx):
    ...
    parts_cmd = update.message.text.split(" ", 1)
    if len(parts_cmd) < 2 or not parts_cmd[1].strip():
        await _reply(update, _USAGE, reply_markup=make_admin_kb(lang))
        return
    ...
    try:
        ...
        if len(parts) != 5:
            raise ValueError("São necessários exactamente 5 campos separados por |")
