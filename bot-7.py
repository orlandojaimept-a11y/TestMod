"""
bot.py — Loja Digital  [hardened v3]

Melhorias vs v2 (bot-6.py):
  - Global error handler do PTB: captura exceções não tratadas em qualquer handler
  - telegram_webhook: try/except para updates malformados (evita crash do processo)
  - Rate limiter com TTL: _rate_buckets é purgado periodicamente (sem memory leak)
  - IPN: validação reforçada de payment_id antes de qualquer query à DB
  - cmd_addproduto: tratamento de erro após add_product (estava ausente no bot-5)
  - cmd_rmproduto: idem para remove_product
  - cmd_comprar: save_order falha → pagamento já foi criado; avisa user corretamente
  - Todos os handlers têm try/except global como fallback final
  - Timeouts explícitos nas chamadas ao bot (send_message, reply_text)
"""
...
# ── Rate limiter com purga periódica (evita memory leak) ──────────────────────

_RATE_WINDOW  = 60       # segundos
_RATE_MAX     = 5        # pedidos por janela
_PURGE_EVERY  = 300      # purgar buckets inativos a cada 5 minutos
...
def _check_rate(user_id: int) -> bool:
    ...
    # Purgar entradas expiradas de TODOS os buckets periodicamente
...
def _kb(update: Update):
    ...
    return make_admin_kb(lang) if _is_admin(update) else make_main_kb(lang)

async def _reply(update: Update, text: str, **kwargs):
    """Wrapper de reply_text com tratamento de erro silencioso."""
    try:
...
# ── Global error handler (PTB) ────────────────────────────────────────────────

async def error_handler(update: object, context) -> None:
    """Captura qualquer exceção não tratada num handler PTB."""
    ...
    if isinstance(update, Update) and update.message:
        ...
        try:
            ...
            pass  # Se nem isto funcionar, não há mais nada a fazer
...
async def telegram_webhook(request: Request):
    ...
    try/except garante que um update malformado ou erro inesperado
    nunca derruba o processo — retorna 200 para o Telegram não reenviar.
    """
    try:
        ...
        # Retorna 200 para evitar que o Telegram reenvie indefinidamente
...
async def nowpayments_ipn(request: Request):
    ...
    # Ignorar statuses intermédios silenciosamente
    if payment_status not in ("finished", "confirmed"):
        return Response(status_code=200)

    # payment_id vazio → rejeitar antes de tocar na DB
    if not payment_id:
        log.error("IPN: payment_id ausente ou vazio.")
        return Response(status_code=400)

    # Comprimento razoável para um payment_id (evita IDs patológicos)
    ...
    # Idempotência: já processado → ignorar sem erro
    ...
    try:
        ...
        # Pagamento foi marcado mas não conseguimos entregar o produto.
        # Retorna 500 para o NowPayments reenviar o IPN e tentarmos de novo.
        return Response(status_code=500)
    ...
    if product:
        try:
            ...
            # Utilizador bloqueou o bot ou chat não existe — não é retriável
...
async def cmd_start(update: Update, ctx):
    lang = get_lang(update)
    await _reply(update, t(lang, "start"), reply_markup=_kb(update))
...
async def cmd_comprar(update: Update, ctx):
    ...
    try:
        ...
        # Pagamento já foi criado na NowPayments — informar o user para não tentar de novo
...
async def cmd_ajuda(update: Update, ctx):
    lang = get_lang(update)
    await _reply(update, t(lang, "help"), reply_markup=_kb(update))
...
async def cmd_botmaker(update: Update, ctx):
    lang = get_lang(update)
    await _reply(update, t(lang, "botmaker"), reply_markup=_kb(update))
...
# Botões do Reply Keyboard — aceita TODOS os textos traduzidos via regex
